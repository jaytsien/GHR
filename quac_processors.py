...
def quac_convert_example_to_features(
    ...
    # )
    truncated_query = tokenizer.encode(
        example.question_text, add_special_tokens=False)
...
def quac_convert_examples_to_features(
    ...
    if return_dataset == "pt":
        ...
        all_yes_no = torch.tensor([f.yes_no for f in features], dtype=torch.long)
        all_follow_up = torch.tensor([f.follow_up for f in features], dtype=torch.long)
        did = []
        index = -1
        pre= ""
        for f in features:
            now = f.qas_id.split("#")[0]
            if now!=pre:
                index += 1
                pre = now
            did.append(index)
        if is_training:
            np.save("datasets/did_index.npy", np.array(did))

        all_did = torch.tensor(did, dtype=torch.long)

        if not is_training:
            all_feature_index = torch.arange(all_input_ids.size(0), dtype=torch.long)
            dataset = TensorDataset(
                all_input_ids, all_attention_masks, all_token_type_ids, all_feature_index, all_cls_index, all_p_mask, all_did
            )
        else:
...
class QuacResult:
    ...
    def __init__(self, unique_id, start_logits, end_logits, no_answer_probs, yes_no_probs, follow_up_probs):
        self.unique_id = unique_id
