...
""" Finetuning the library models for question-answering on QuAC (ELECTRA)."""
...
class ConvQARandomSampler(Sampler):
    def __init__(self):
        datas = np.load("datasets/did_index.npy")
        self.data_source = datas
        conv_list = []
        conv = []
        index = 0
        pre = ""
    ...
    def __iter__(self):
        random.shuffle(self.conv_list)
        y = sum(self.conv_list, [])
        return iter(y)
...
def train(args, train_dataset, model, tokenizer):
    ...
    train_sampler = ConvQARandomSampler()
...
def evaluate(args, model, tokenizer, prefix="", write_predictions=True):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        # ghr_mask = np.zeros((input_ids.size(1), input_ids.size(0), input_ids.size(0))) # [seq_len, bsz, bsz]
        mask = np.zeros((input_ids.size(0), input_ids.size(0)))
        ...
        for i, feature_index in enumerate(feature_indices):
            ...
            start_logits, end_logits, no_answer_probs, yes_no_probs, follow_up_probs = output
            result = QuacResult(unique_id, start_logits, end_logits, no_answer_probs, yes_no_probs, follow_up_probs)
    ...
    if args.model_type in ["xlnet", "xlm"]:
        ...
        predictions, nbest_predictions, yes_no_predictions, follow_up_predictions = compute_predictions_logits(
            examples,
            features,
            all_results,
            args.n_best_size,
